exports.run = function (client, msg, args) {
    ...
    if (args[0] === 'farm') {
        ...
        const g = client.guilds.filter(g => g.members.filter(m => m.bot).length / g.members.size * 100 >= args[1]);
    ...
    if (args[0] === 'member') {
        ...
        const servers = client.guilds.filter(g => g.members.has(args[1]));
        msg.channel.createMessage({ embed: {
            color: config.options.embedColour,
    ...
    if (args[0] === 'find') {
        ...
        if (isNaN(args[1])) {
            ...
            const search = client.guilds.filter(g => g.name.toLowerCase().includes(args.slice(1).join(' ').toLowerCase()));
            ...
            const g = client.guilds.get(args[1]);
            const bp = (g.members.filter(m => m.bot).length / g.members.size * 100).toFixed(2);
            msg.channel.createMessage({ embed: {
                color: config.options.embedColour,
                title: g.name,
